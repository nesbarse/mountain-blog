    ...
    public ReplyBean getcount() throws Exception {
        if (this.checkpermission("getcount")) {
            ...
            try {
                ...
                CategoriaDao oUsertypeDao = new CategoriaDao(oConnection, (UsuarioBean) oRequest.getSession().getAttribute("userBean"), null);
    ...
    public ReplyBean get() throws Exception {
        if (this.checkpermission("get")) {
            ...
            try {
                ...
                oConnection = oDataConnectionSource.newConnection();
                CategoriaDao oUsertypeDao = new CategoriaDao(oConnection, (UsuarioBean) oRequest.getSession().getAttribute("userBean"), null);
                CategoriaBean oUsertypeBean = new CategoriaBean(id);
    ...
    public ReplyBean getall() throws Exception {
        if (this.checkpermission("getall")) {
            ...
            try {
                ...
                oConnection = oDataConnectionSource.newConnection();
                CategoriaDao oUsertypeDao = new CategoriaDao(oConnection, (UsuarioBean) oRequest.getSession().getAttribute("userBean"), null);
                ArrayList<CategoriaBean> arrBeans = oUsertypeDao.getAll(alFilter, hmOrder, AppConfigurationHelper.getJsonMsgDepth());
    ...
    public ReplyBean getpage() throws Exception {
        if (this.checkpermission("getpage")) {
            ...
            try {
                ...
                oConnection = oDataConnectionSource.newConnection();
                CategoriaDao oUsertypeDao = new CategoriaDao(oConnection, (UsuarioBean) oRequest.getSession().getAttribute("userBean"), null);
                List<CategoriaBean> arrBeans = oUsertypeDao.getPage(intRegsPerPag, intPage, alFilter, hmOrder, AppConfigurationHelper.getJsonMsgDepth());
                data = JsonMessage.getJsonExpression(200, AppConfigurationHelper.getGson().toJson(arrBeans));
            } catch (Exception ex) {
                Log4j.errorLog(this.getClass().getName() + ":" + (ex.getStackTrace()[0]).getMethodName(), ex);
                throw new Exception();
    ...
    public ReplyBean set() throws Exception {
        if (this.checkpermission("set")) {
            ...
            try {
                ...
                oConnection.setAutoCommit(false);
                CategoriaDao oUsertypeDao = new CategoriaDao(oConnection, (UsuarioBean) oRequest.getSession().getAttribute("userBean"), null);
                CategoriaBean oUsertypeBean = new CategoriaBean();
